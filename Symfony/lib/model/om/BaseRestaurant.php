...
abstract class BaseRestaurant extends BaseObject  implements Persistent {
  ...
  const PEER = 'RestaurantPeer';
	...
	/**
	 * Initializes internal state of BaseRestaurant object.
	 * @see        applyDefaults()
	 */
	 ...
	 * @return     Restaurant The current object (for fluent API support)
	 */
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 4; // 4 = RestaurantPeer::NUM_COLUMNS - RestaurantPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = RestaurantPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
    ...
    foreach (sfMixer::getCallables('BaseRestaurant:delete:pre') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseRestaurant:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseRestaurant:save:pre') as $callable)
    {
    ...
    if ($this->isNew() && !$this->isColumnModified(RestaurantPeer::CREATED_AT))
    {
      $this->setCreatedAt(time());
    }

    if ($this->isModified() && !$this->isColumnModified(RestaurantPeer::UPDATED_AT))
    {
      $this->setUpdatedAt(time());
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BaseRestaurant:save:post') as $callable)
    {
      call_user_func($callable, $this, $con, $affectedRows);
    }

			RestaurantPeer::addInstanceToPool($this);
			return $affectedRows;
		} catch (PropelException $e) {
			$con->rollBack();
			throw $e;
  ...
  {
    if (!$callable = sfMixer::getCallable('BaseRestaurant:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BaseRestaurant::%s', $method));
    }
